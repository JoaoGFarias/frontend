Feature: Ordering of products on Demoblaze e-commerce

  Scenario: Order a product
    Given I am on the Demoblaze Home page
#    When select the "Sony vaio i5" product in the "Laptop" category
#    And select the "Dell i7 8gb" product in the "Laptop" category
#    And I go the cart
#    And I delete the "Dell i7 8gb" from the cart
#    And I complete the order, passing any data
#    Then the order is confirmed
#    And the expected total purchase total is the sum of the ordered products
